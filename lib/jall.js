...
function Localize(translations, pathToStrings, dateFormats, defaultLocale) {
    ...
    function getTranslations(currDir, translations, strings) {
        if(fs.existsSync(currDir)) {
            ...
            if(typeof(pathToStrings) === "string") {
                translationPath = path.join(currDir, pathToStrings);
            } else {
                translationPath = path.join(currDir, "translations");
            }
    ...
    // is a thin automatic substitution wrapper around ``buildString``. It
    // looks up the required translated string and executes ``buildString`` on
    // that
    this.translate = function() {
        ...
        if(missingTranslationThrow && typeof(newText) !== "string") {
            ...
            newText = translations[arguments[0]] && translations[arguments[0]][defaultLocale] ?
                translations[arguments[0]][defaultLocale] : arguments[0];
        }
